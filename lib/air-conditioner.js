...
AirConditioner.prototype = {
	...
	const newtargettemp = this.currentTargetTemp(opMode);

	if (this.targettemp != newtargettemp) {
		this.log('Setting target temperature:', newtargettemp);
	...
	const newtargettemp = this.currentTargetTemp(this.currentDeviceState[State.OpMode]);

	if(this.init){
		this.log('Init');
		const temp = this.currentDeviceState[State.TempNow];
		this.targettemp = temp;
		this.mintemp = (temp - (this.tolerance * 2));
		this.maxtemp = (temp + (this.tolerance * 2));
		this.init = false;
	} else if(this.checkTargetTemp && Math.abs(this.targettemp - newtargettemp) > this.tolerance){
		this.log('Updating target temperature:', newtargettemp);
		this.targettemp = newtargettemp;
		this.acService.getCharacteristic(Characteristic.TargetTemperature).setValue(this.targettemp);
	}
